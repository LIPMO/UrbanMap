...
function initMap(){
  ...
  navigator.geolocation.watchPosition(pos => {
    ...
    if (speed > MAX_DISPLAY_SPEED) speed = MAX_DISPLAY_SPEED
    ...
    socket.emit("update", { lat, lon, speed, vmax })
  },
  () => alert("GPS désactivé"),
  { enableHighAccuracy: true })
}

socket.emit("join", { pseudo: localStorage.getItem("pseudo"), vmax })
...
socket.on("users", data => {
  ...
  Object.keys(data).forEach(id => {
    ...
    if (!riders[id]) {
      ...
      riders[id].card.setLngLat([data[id].lon, data[id].lat])
